...
import { LoanReport, fakeLoanReports } from '@/constants/mock-api-loans-report';

export default function ListingPage() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [cooperatives, setCooperatives] = useState<LoanReport[]>([]);
  ...
  const fetchCooperatives = async () => {
    try {
      const data = await fakeLoanReports.getAll({
        page,
        limit: pageLimit,
        search,
    ...
    <PageContainer scrollable>
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          <Heading
            title={`Data Laporan Pinjaman (${totalCooperative})`}
            description="Kelola data laporan pinjaman"
          />
        </div>
        <Separator />
        <UsersTable
